...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("error change address %d, status is not %s", number, ParcelStatusRegistered)
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("error change number %d cannot be deleted is not status %s", number, ParcelStatusRegistered)
	}
